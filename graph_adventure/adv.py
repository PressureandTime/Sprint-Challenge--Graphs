...
def depth_first_recursive(current_room,
                          prev_direction=None,
                          prev_room_id=None):
    ...
    if prev_direction:
        rooms[current_room.id][
            opposite_directions[prev_direction]] = prev_room_id
    for next_room in rooms[current_room.id]:
        if rooms[current_room.id][next_room] == '?':
            player.travel(next_room)
            rooms[current_room.id][next_room] = player.currentRoom.id
            traversalPath.append(next_room)
            depth_first_recursive(player.currentRoom, next_room,
                                  current_room.id)
            return


def breadth_first_search(current_room_id):
    queue = Queue()
    ...
    while queue.size() > 0:
        path = queue.dequeue()
        current_id = path[-1]
        if current_id not in rooms_visited:
            values = list(rooms[current_id].values())
            if '?' in values or len(roomGraph) == len(visited):
                return path
            rooms_visited.add(current_id)
            for next_room in values:
                if (next_room == 0
                        or next_room) and next_room not in rooms_visited:
                    new_path = list(path)
                    new_path.append(next_room)
                    queue.enqueue(new_path)


def traverse_the_rooms():
    while len(roomGraph) != len(visited):
        depth_first_recursive(player.currentRoom)
        path = breadth_first_search(player.currentRoom.id)
        for index in range(0, len(path) - 1):
            current_room_id = path[index]
            next_room_id = path[index + 1]
            current_room = rooms[current_room_id]
            keys = list(current_room.keys())
            values = list(current_room.values())
            direction = keys[values.index(next_room_id)]
